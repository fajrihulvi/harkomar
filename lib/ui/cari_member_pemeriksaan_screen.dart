...
class CariMemberPemeriksaanScreen extends StatefulWidget {
  @override
  _CariMemberPemeriksaanScreenState createState() =>
      _CariMemberPemeriksaanScreenState();
}

class _CariMemberPemeriksaanScreenState
    extends State<CariMemberPemeriksaanScreen> {
  ...
  void setBody(BuildContext context, String query) async {
    await model.getberitaAcara(
        Provider.of<User>(context).token, "Pemeliharaan", query);
    setState(() {
                  ...
                  Row(
                    ...
                    children: <Widget>[
                      RaisedButton(
                          ...
                          onPressed: () async {
                            if (beritaAcara.lat.isNotEmpty &&
                                beritaAcara.long.isNotEmpty) {
                              MapsLauncher.launchCoordinates(
                                  double.parse(beritaAcara.lat),
                                  double.parse(beritaAcara.long));
                            } else {
                              Flushbar(
                                icon: Icon(Icons.info, color: colorWhite),
                                title: "Informasi",
                                message: "Latitude, Longitude tidak ditemukan.",
